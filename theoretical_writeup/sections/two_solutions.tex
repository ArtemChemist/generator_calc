...
Bateman proposed an analytical solution to arbitrarily long sequential decay chain. Equation \eqref{eq:bateman_original} describes a typical situation in an isotope generator: pure parent isotope at time 0 with no daughters, and no generative processes. In other words, what happens after parent isotope is removed from irradiation facility and chemically purified.
\begin{equation}
	Q_{n}(t) = Q_n(0)\cdot\prod_{j=1}^{n-1}k_{j,j+1}\cdot\sum_{j=1}^{n}\frac{e^{-k_{j}t}}{\prod_{p=1, p\ne j}^{n}(k_{p}-k{j})}
